          ...
          <div className="bg-white rounded-[2rem] md:rounded-[3rem] w-full max-w-2xl shadow-2xl overflow-hidden relative flex flex-col max-h-[85vh] border border-white/20">
            <div className="px-5 py-5 md:px-8 md:py-8 border-b border-gray-100 flex items-center justify-between shrink-0 bg-gray-50/50">
               <div className="flex items-center gap-3 md:gap-4">
                  ...
                  <div className="min-w-0">
                    <h3 className="text-base md:text-xl font-black text-[#004A74] uppercase tracking-tight truncate max-w-[180px] md:max-w-md">Article Metadata</h3>
                    <p className="text-[8px] md:text-[9px] font-bold text-gray-400 uppercase tracking-widest">Global Repository Identity</p>
                  </div>
               </div>
               <button onClick={() => setPreviewItem(null)} className="p-1.5 md:p-2 hover:bg-red-50 text-gray-400 hover:text-red-500 rounded-full transition-all">
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar p-5 md:p-10 space-y-6 md:space-y-8">
               <div className="space-y-3 md:space-y-4">
                  <h1 className="text-lg md:text-2xl font-black text-[#004A74] leading-tight uppercase">{previewItem.title}</h1>
                  <div className="flex items-center gap-3">
                    <span className="px-2 py-0.5 md:px-3 md:py-1 bg-[#FED400] text-[#004A74] text-[8px] md:text-[10px] font-black uppercase tracking-widest rounded-full">{previewItem.year || 'n.d.'}</span>
                    <p className="text-xs md:text-sm font-bold text-gray-600 italic">{previewItem.authors?.map(a => a.name).join(', ')}</p>
                  </div>
               </div>

               <div className="p-4 md:p-6 bg-blue-50/30 rounded-2xl md:rounded-3xl border border-blue-100/50 space-y-3 md:space-y-4">
                  <div className="flex items-center gap-2 text-[8px] md:text-[10px] font-black text-[#004A74] uppercase tracking-[0.2em]">
                     <Info className="w-3 h-3 md:w-4 md:h-4" /> Validity Warning
                  </div>
               ...
               {previewItem.doi && (
                  <div className="flex items-center gap-2 text-xs font-bold text-gray-400">
                     ...
                     <span className="font-mono text-[9px] md:text-[11px] text-[#004A74] opacity-70">{previewItem.doi}</span>
                  </div>
               )}
            </div>
            ...
            <div className="px-5 py-5 md:px-10 md:py-8 border-t border-gray-100 flex items-center justify-between gap-2 md:gap-3 bg-gray-50/30">
               <button 
                 ...
                 className="px-3 py-2 md:px-6 md:py-4 text-gray-400 bg-white border border-gray-100 rounded-xl md:rounded-2xl text-[8px] md:text-[9px] font-black uppercase tracking-widest shadow-sm hover:text-[#004A74] transition-all flex items-center gap-2"
               >
               ...
               <div className="flex items-center gap-2 md:gap-3">
                 ...
                 <button 
                   ...
                   className="px-3 py-2 md:px-8 md:py-4 bg-[#004A74] text-white rounded-xl md:rounded-2xl text-[8px] md:text-[10px] font-black uppercase tracking-widest shadow-xl shadow-[#004A74]/10 hover:scale-105 active:scale-95 transition-all flex items-center gap-2"
                 >
                   <Save className="w-3.5 h-3.5 md:w-4 md:h-4" /> <span className="hidden sm:inline">Archive Now</span><span className="sm:hidden">Archive</span>
                 </button>
               </div>
            </div>
          </div>
